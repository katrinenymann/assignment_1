...
```{r ex2, include = FALSE}
ggplot(df_train, aes(VISIT, CHI_MLU, color = Diagnosis)) + stat_summary(fun.y = mean, geom = "point") + stat_summary(fun.y = mean, geom = "line", aes(group=Diagnosis))

#With a regression line
ggplot(df_train, aes(VISIT, CHI_MLU, color = Diagnosis)) + geom_smooth(method = "lm")
...
#We intriduce polynomials.
```

```{r ex2 growth curve, include = FALSE}
#CHI_MLU ~ VISIT + VISIT + I(VISIT)^2
```
...
```{r}
#Given you initial verbalIQ you might learn more on average for each visit. This assumes that verbalIQ and Diagnosis are independant. 
#CHI_MLU ~ VISIT + DIAGNOSIS + (VerbalIQ*VISIT)
