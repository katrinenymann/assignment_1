...
m1 <- lmer(CHI_MLU ~ Visit*Diagnosis*verbalIQ1 + (1 + Visit|Child.ID), data = df_train)

m2 <- lmer(CHI_MLU ~ Visit*Diagnosis + verbalIQ1 + Ethnicity + MOT_MLU + (1 + Visit|Child.ID), data = df_train)

#- calculate performance of the model on the training data: root mean square error is a good measure. (Tip: google the function rmse())
predict_1 <- predict(m1)
predict_2 <- predict(m2)

rmse(df_train$CHI_MLU[!is.na(df_train$CHI_MLU)], predict_1)
rmse(df_train$CHI_MLU[!is.na(df_train$CHI_MLU)], predict_2)

"RMSE the amount of error the model makes when predicting CHI_MLU form known data. Can be compared to the standard deviation sd() to see wether it is big or acceptable." 
# cvms package does the cross-validation 
...
#- test the performance of the models on the test data (Tips: google the functions "predict()")
df_test <- subset(df_test, (!is.na(df_test$CHI_MLU)))
...
rmse(df_test$CHI_MLU, predict_test_1)
rmse(df_test$CHI_MLU, predict_test_2)
