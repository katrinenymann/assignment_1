...
``` r
#We rename the subjects column
demo_train <- rename(demo_train, SUBJ = Child.ID) #We rename the column so they are the same

#WE rename the Visit column
...
``` r
#We want to make the values in visit the same
token_train$VISIT <- gsub("[a-zA-Z ]", "", token_train$VISIT)
token_train$VISIT <- gsub("[.]", "", token_train$VISIT)
LU_train$VISIT <- gsub("[a-zA-Z ]", "", LU_train$VISIT)
LU_train$VISIT <- gsub("[.]", "", LU_train$VISIT)
```
...
``` r
#We remove punctuation from SUBJ
...
``` r
#We will make a dataset only including the variables we need
pacman::p_load(tidyverse)

demo <- select(demo_train, SUBJ, VISIT, Diagnosis, Ethnicity, Gender, Age, ASD_level = ADOS, nonVerbalIQ = MullenRaw, VerbalIQ = ExpressiveLangRaw, Socialization)

LU <-  select(LU_train, SUBJ, VISIT, MOT_MLU, CHI_MLU)

token <- select(token_train, SUBJ, VISIT,  types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
``` r
#We will merge the data together into one dataframe
#We can only do two at a time
demo_LU <- merge(demo, LU)
#And then all together
df_train <- merge(demo_LU, token)
```
...
``` r
#We want unique data for visit 1
#We filter out visit 1 data
visit_1 <- filter(df_train, VISIT == 1)

#We select only the variables we wish. We also rename
visit_1 <- select(visit_1, SUBJ, ASD_level_1 = ASD_level, nonVerbalIQ_1 = nonVerbalIQ, VerbalIQ_1 = VerbalIQ, Socialization_1 = Socialization)

#We merge it together
df_train <- merge(df_train, visit_1)
```
...
``` r
#We will anonymize our participants 
df_train$SUBJ <- as.factor(df_train$SUBJ)
ID <- (1:61)

#We use levels so it will go the the next name when giving new number
levels(df_train$SUBJ) <- ID

#We will give sensible names
#We change gender values
df_train$Gender <- ifelse(df_train$Gender == 1, "F", "M") 
#We change the diagnosis names
df_train$Diagnosis <- ifelse(df_train$Diagnosis == "A", "ASD", "TD")
