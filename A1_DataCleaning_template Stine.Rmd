...
#in outout you can change to a mark down file which looks nicer on github
---
...
merge1 <- merge(demo, LU)
df_train <- merge(merge1, token)
...
```{r}
visit1 <- filter(df_train, VISIT == 1)

visit1 <- visit1 %>% select(SUBJ, ASD_level_1 = ASD_level, nonVerbalIQ_1 = nonVerbalIQ, VerbalIQ_1= VerbalIQ, Socialization_1= Socialization)

df_train <- merge(df_train, visit1)
```
...
```{r}
# 1
MLU_2.7 <- filter(df_train, CHI_MLU > 2.7)

# 2
MLU_1.5 <- filter(df_train, CHI_MLU < 1.5 & VISIT == 1)

# 3
not_all_trials <- group_by(df_train, SUBJ) %>% summarize("number"=n()) %>% filter(number<6)
```
...
```{r}
most_words <- filter(df_train, VISIT == 6) %>% arrange(desc(tokens_CHI))

least_words <- filter(df_train, VISIT == 1) %>% arrange(tokens_CHI)
...
```{r}
ASD_MLU_tokens <- subset(df_train, Diagnosis=="ASD") %>% select(MOT_MLU, CHI_MLU, tokens_CHI, tokens_MOT)

# It excludes it 
ASD_MLU_tokens <- subset(df_train, Diagnosis=="ASD") %>% select(MOT_MLU, CHI_MLU, tokens_CHI, tokens_MOT, tokens_MOT)
```
...
```{r}
# 1 
df_train$MEAN <- mean(df_train$tokens_CHI)

# 2

MEAN_SUBJ <- group_by(df_train, SUBJ) %>% summarize("MEAN_SUBJ"=mean(tokens_CHI))

#To make it put each mean on each subject 
df_train <- merge(df_train, MEAN_SUBJ)
